# Conflict Network (RQ4) — corpus-wide sweep (tranches 1-2), single annotator

Coded from Moran (1992) per `registry/conflict_codebook.md`,
corpus-wide (tranche 1: the five classic theatres; tranche 2: full
hostile-language sweep). Independently recoded from Rainey (2015):
cross-edition kappa 0.87 (`coding_reliability.md`) - but both passes
share one annotator, so the PLAN.md 7 second-coder pass is still
required before these edges enter the base analysis.

- letters coded: 40; edges: 63 (59 accusations, 4 alliance)
- actors: 41

## Most-accused

| actor | accused in n letters |
|---|---|
| abdiasirta | 8 |
| aziru | 7 |
| sons-of-abdiasirta | 6 |
| milkilu | 4 |
| yappahhadda | 3 |
| sons-of-labaya | 2 |
| tagi | 2 |
| zimreddi | 2 |

## Betweenness in the (undirected) conflict graph

| actor | betweenness | accused_by_n | accuses_n |
|---|---|---|---|
| ribhadda | 108.0 | 0 | 26 |
| aziru | 104.0 | 7 | 1 |
| ilirapih | 56.0 | 0 | 3 |
| etakkama | 45.0 | 1 | 1 |
| abdiheba | 42.5 | 1 | 9 |
| abimilku | 33.0 | 0 | 4 |
| biryawaza | 33.0 | 1 | 2 |
| suwardata | 31.0 | 1 | 1 |
| surata | 18.0 | 1 | 0 |
| biridiya | 10.0 | 0 | 2 |

## Reading

Even in this partial tranche the RQ4 hypothesis pattern is visible:
the conflict graph's central figures are the *accused bridges* —
Aziru (accused from Byblos, Tyre, and implicated at Damascus) and the
Milkilu/Tagi/sons-of-Labaya bloc — not the loudest accusers. The
inter-vassal politics invisible in the correspondence layer (which
has literally zero vassal-vassal edges beyond two) is the dominant
structure here. Full coverage and second coding in tranche 2.
