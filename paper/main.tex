...
splits cleanly by layer: distance decays mention and conflict ties ---
neighbors name and accuse neighbors --- while epistolary volume to the
court shows no robust distance effect, a contrast that survives
compositional drops and control for proximity to Egyptian administrative
centers. We release the actor
...
pharaonic throne names, each with a justification.
...
against Pleiades (31/55 automated; regions without ancient fixed points
remain unlocated by design), and every coordinate entering a distance
test was hand-verified against the Pleiades bulk dump --- a necessary
step, since classical gazetteers are homonym-rich (Pella, Arca, Tyros,
and ``Syria'' all have distant namesakes). The petrographic
...
\subsection{Geography: distance structures mentions and conflict, not
correspondence}
\label{sec:geography}
...
conflict network in space makes the result visible
(Figure~\ref{fig:conflictmap}): accusations run between neighbors ---
along the Phoenician coast, across the Beqaa, and within the crowded
hill country between Gezer, Gath, Shechem, and Jerusalem --- while the
one long-range structure is the Qadesh--Damascus axis of the Hittite
crisis. Correspondence with the court behaves differently. The raw
correlation between a vassal's letter count and distance from Akhetaten
is $\rho=-0.32$ ($p=0.07$), and it is not robust: a leave-one-polity-out
jackknife retains nominal significance in only 7 of 24 deletions;
dropping the polities with political reasons to write little ---
Hatti-aligned Qadesh and Sidon (EA~174--176, 148--149) and
quasi-independent Ugarit --- leaves $\rho=-0.18$ ($p=0.32$); and in a
partialled design against hand-coded covariates --- distance to the
nearest Egyptian administrative center (Gaza, Joppa, Beth-Shean, Kumidi,
Ṣumur; Figure~\ref{fig:map}; cf.\ \citealp{Morris2005}), coastal
access, Hatti alignment --- nothing predicts epistolary volume, court
distance included (partial $r=-0.26$, $p=0.17$). The sign stays
negative in every variant, so a weak true decay is not excluded; but
the defensible statement is the pre-registered one: geography
structures whom the letters \emph{talk about}, not who talks to Egypt.
(The repository documents how an apparently strong correspondence decay
in an earlier draft traced to gazetteer homonym mismatches and a
rank-tie artifact --- the coordinate verification described in \S2
exists because of that episode.)
...
\begin{figure}[tb]
\centering
\includegraphics[width=0.72\textwidth]{figures/fig7_conflict_map.pdf}
\caption{The conflict network in space: accusation (red) and alliance
(blue) edges between located polities, aggregated over letters.
Accusations run between neighbors --- the Phoenician coast, the
Gezer--Gath--Shechem--Jerusalem hill country --- with the
Qadesh--Damascus axis of the Hittite crisis as the one long-range
structure. Edges involving unlocated actors (collectives, the ʿApiru)
are omitted.}
